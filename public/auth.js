// Removed sensitive token — do not store secrets in source code
...
const firebaseConfig = {
  ...
  databaseURL: "https://gamepathver1-default-rtdb.firebaseio.com", // Added Code
...
function renderLibrary(libraryId, libraryData) {
  ...
  if (!libraryElement) {
    ...
    libraryElement.querySelector(".library-name").textContent =
      libraryData.name;
    libraryElement.querySelector(".library-labels").innerHTML =
      libraryData.labels.map(
...
if (libraryForm) {
  libraryForm.addEventListener("submit", async function (e) {
    ...
    const labels = Array.from(labelInputs)
